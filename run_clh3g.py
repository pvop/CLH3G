...
from bert.transformer_builder_title_vector_contra import build_model
...
def read_dataset(args, path):
    ...
    for i in range(len(data)):
        if i%10000==0:
            print ("Dealed {}, Total {}".format(i, len(data)), flush=True)
        if len(data[i])!=3:
            continue
        if not "".join(data[i][1:]).strip() or not data[i][0].strip():
            continue
        title_article = data[i][1:]
        summ = data[i][0]
        article = []
        title = []
        unk_sets = {}
        unk_num = len(args.vocab) - 1
        for index, sentence in enumerate(title_article):
            if index>=1:
                ...
                sentence = [summ] + sentence.split("|")
                for single_tit in sentence:
                    tmp_tit = []
                    all_words = args.tokenizer.tokenize(single_tit)
                    for t in all_words:
                        t_id = args.vocab.get(t)
                        if t_id == UNK_ID:
                            if unk_sets.get(t) is not None:
        ...
        if len(article)==0 or len(title)==0:
            continue
        article = article[0:args.seq_length] + [PAD_ID] * (args.seq_length - len(article))
        for i in range(len(title)):
            title[i] = [CLS_ID] + title[i]
            title[i] = title[i][0:args.title_seq_length]+ [PAD_ID] * (args.title_seq_length - len(title[i]))
        tgt = tgt[0:args.tgt_length] + [PAD_ID] * (args.tgt_length - len(tgt))
        dataset.append([article, title, tgt, unk_sets])
    return dataset

def batch_loader(args, trainset, proc_id, proc_num, test=False):
    procset = trainset[proc_id::proc_num]
    article = torch.LongTensor([example[0] for example in procset])
    title = []
    segment_length = []
    mask_tv = []
    mask_contra = []
    for example in procset:
        mask_tv.append([])
        mask_contra.append([])
        title.append(example[1])
        segment_length.append([len(example[1]) - 1])
        mask_tv[-1].append(0)
        mask_tv[-1].extend([1] * (len(example[1]) - 1))
        x = [0] * len(example[1])
        a, b = random.sample(range(len(example[1])), 2)
        x[a] = 1
        x[b] = 1
        mask_contra[-1].extend(x)
    segment_length = torch.LongTensor(segment_length)
    tgt = torch.LongTensor([example[2] for example in procset])
    unk_sets = [example[3] for example in procset]
    instances_num = article.size()[0]
    if test:
    ...
    while True:
        ...
        if i*batch_size>=instances_num and test:
            break
        art_batch = article[i * batch_size: (i + 1) * batch_size, :]
        title_batch_tmp = title[i * batch_size: (i + 1) * batch_size]
        title_batch = []
        for tit in title_batch_tmp:
            title_batch.extend(tit)
        title_batch = torch.LongTensor(title_batch)
        segment_length_batch = segment_length[i * batch_size: (i + 1) * batch_size, :]
        mask_tv_batch_tmp = mask_tv[i * batch_size: (i + 1) * batch_size]
        mask_tv_batch = []
        for tv in mask_tv_batch_tmp:
            mask_tv_batch.extend(tv)
        mask_tv_batch = torch.BoolTensor(mask_tv_batch).unsqueeze(1)
        mask_contra_batch_tmp = mask_contra[i * batch_size: (i + 1) * batch_size]
        mask_contra_batch = []
        for contra in mask_contra_batch_tmp:
            mask_contra_batch.extend(contra)
        mask_contra_batch = torch.BoolTensor(mask_contra_batch).unsqueeze(1)
        tgt_batch = tgt[i * batch_size: (i + 1) * batch_size]
        unk_sets_batch = unk_sets[i * batch_size: (i + 1) * batch_size]
        i += 1
        yield art_batch, title_batch, segment_length_batch, mask_tv_batch, mask_contra_batch, tgt_batch, unk_sets_batch
...
def train_model(args, gpu_id, rank, loader, model, optimizer, scheduler, steps):
    ...
    while True:
        ...
        if steps == total_steps + 1:
            break
        art, title, segment_length, mask_tv, mask_contra, tgt, _ = next(loader)
        if gpu_id is not None:
            art = art.cuda(gpu_id)
            title = title.cuda(gpu_id)
            segment_length = segment_length.cuda(gpu_id)
            mask_tv = mask_tv.cuda(gpu_id)
            mask_contra = mask_contra.cuda(gpu_id)
            tgt = tgt.cuda(gpu_id)
        loss_1, loss_2 = model(art, title, segment_length, mask_tv, mask_contra, tgt)
        loss = loss_1 + loss_2
        ...
        if steps % args.report_steps == 0 and \
            ...
            loss_1 = total_loss_1 / args.report_steps
            loss_2 = total_loss_2 / args.report_steps
            print ("Step {}, Total Loss is {}, Gen Loss is {}, Contra loss is {}, lr is {}".
                   format(steps, loss, loss_1, loss_2, optimizer.state_dict()['param_groups'][0]['lr']), flush=True)
            total_loss = 0
            total_loss_1 = 0
            total_loss_2 = 0
...
def cal_rouge_blue(args, candidate_datas, data_path):
    ...
    gold_datas = [" ".join(x.split("\t")[0]) for x in gold_datas if len(x.split("\t"))==3]
...
def evaluate(args,model, gpu_id, dataset, data_path):
    ...
    datas = [x.split("\t", 1)[1] for x in datas if len(x.split("\t"))==3]
    t1 = time.time()
    for index, (art_batch, title_batch, segment_length_batch, mask_tv_batch, mask_contra_batch, tgt_batch, unk_sets) in enumerate(batch_loader(args, dataset, 0, 1, test=True)):
        if index%100==0:
            print ("Dealed {}, Total {}".format(index*args.batch_size, len(dataset)), flush=True)
        art_batch = art_batch.to(gpu_id)
        mask_tv_batch = mask_tv_batch.to(gpu_id)
        mask_contra_batch = mask_contra_batch.to(gpu_id)
        title_batch = title_batch.to(gpu_id)
        segment_length_batch = segment_length_batch.to(gpu_id)
        with torch.no_grad():
            pred_tokens = model.inference_forward_beam_search(art_batch, title_batch, segment_length_batch, mask_tv_batch, mask_contra_batch)
        for i in range(len(pred_tokens)):
            pred_tokens[i] = "".join([args.vocab.i2w[x] for x in pred_tokens[i][0]])
            x = pred_tokens[i].replace("[UNK]", "").replace("[unused2]", "").replace("##", "")
...
def main():
    ...
    parser.add_argument("--title_seq_length", type=int, default=40,
                        help="Sequence length.")
    ...
    parser.add_argument("--pointer_title_vector", action='store_true',
                        help="whether use pointer title vector")
    parser.add_argument("--decoder_title_vector", action='store_true',
                        help="whether use decoder title vector")
    parser.add_argument("--learning_rate", type=float, default=2e-5,
                        help="Learning rate.")
    parser.add_argument("--contra_lambda", type=float, default=0.0,
