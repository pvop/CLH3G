...
class PegasusModelStackFuse(nn.Module):
    """
    ...
    def __init__(self, args, src_embedding, tgt_embedding, encoder, decoder, target, start_id, vocab_size=None, end_id=None):
        super(PegasusModelStackFuse, self).__init__()
    ...
    def forward(self, article, titles, tgt):
        ...
        tit_encoder_output = self.key_attn(art_encoder_output, tit_encoder_output, article, titles)
        ...
        decoder_output = self.decoder(art_encoder_output, tit_encoder_output, tgt_emb, article, article, st=False)
        loss_info = self.target(art_encoder_output, decoder_output, article, tgt)
        return loss_info
    ...
    def inference_forward_beam_search(self, article, titles):
        ...
        tit_encoder_output = self.get_encoder_output(titles)
        tit_encoder_output = self.key_attn(art_encoder_output, tit_encoder_output, article, titles)
        article = self.tile(article, self.beam_size, dim=0)
        titles = self.tile(titles, self.beam_size, dim=0)
        art_encoder_output = self.tile(art_encoder_output, self.beam_size, dim=0)
        tit_encoder_output = self.tile(tit_encoder_output, self.beam_size, dim=0)
        batch_offset = torch.arange(batch_size, dtype=torch.long, device=article.device)
        ...
        topk_log_probs = (
            torch.tensor([0.0] + [float("-inf")] * (self.beam_size - 1), device=article.device).repeat(batch_size)
        )
        ...
        for step in range(self.tgt_length):
            ...
            decoder_output = self.decoder(art_encoder_output, tit_encoder_output, tgt_emb, article, article, with_cache=True)
            logits = self.target.inference_forward(art_encoder_output,
                                                   decoder_output,
                                                   article)
            ...
            select_indices = batch_index.view(-1)
            art_encoder_output = art_encoder_output.index_select(0, select_indices)
            tit_encoder_output = tit_encoder_output.index_select(0, select_indices)
            article = article.index_select(0, select_indices)
            titles = titles.index_select(0, select_indices)
