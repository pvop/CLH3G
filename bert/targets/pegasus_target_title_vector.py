...
class PegasusTargetTitleVector(nn.Module):
    ...
    def __init__(self, args, vocab_size):
        ...
        self.pointer_atten_1 = nn.Linear(2 * args.hidden_size, args.hidden_size, bias=has_bias)
        self.pointer_atten_2 = nn.Linear(2 * args.hidden_size, args.hidden_size, bias=has_bias)
        self.alpha_prob_linear = nn.Linear(3 * args.hidden_size, 1, bias=has_bias)
    ...
    def pgn_inference(self, encoder_outputs, decoder_outputs, src_input, title_vector):
        tgt_len = decoder_outputs.size(1)
        pointer_weights = torch.matmul(self.pointer_atten_1(
            torch.cat([decoder_outputs, title_vector.repeat(1, decoder_outputs.size(1), 1)], dim=-1)),
                                       self.pointer_atten_2(
            torch.cat([encoder_outputs, title_vector.repeat(1, encoder_outputs.size(1), 1)], dim=-1)).transpose(1, 2)).squeeze(2)
        pointer_prob = torch.softmax(pointer_weights, dim=2)
        encoder_atten_vector = torch.matmul(pointer_prob, encoder_outputs)
        alpha_prob = F.sigmoid(self.alpha_prob_linear(torch.cat([decoder_outputs, encoder_atten_vector, title_vector.repeat(1, decoder_outputs.size(1), 1)], dim=-1)))
    ...
    def mlm(self, encoder_outputs, decoder_outputs, src_input, tgt_mlm, title_vector):
        output_mlm = self.pgn_inference(encoder_outputs, decoder_outputs, src_input, title_vector)
    ...
    def forward(self, encoder_outputs, decoder_outputs, src_input, tgt_mlm, title_vector):
        """
        Args:
        ...
        loss, correct, denominator = self.mlm(encoder_outputs, decoder_outputs, src_input, tgt_mlm, title_vector)

        return loss, correct, denominator
    def inference_forward(self, encoder_outputs, decoder_outputs, src_input, title_vector):
        output_mlm = self.pgn_inference(encoder_outputs, decoder_outputs, src_input, title_vector)
