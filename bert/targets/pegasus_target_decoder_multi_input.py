...
class PegasusTargetDecoderMultiInput(nn.Module):
    """
    BERT exploits masked language modeling (MLM) 
    and next sentence prediction (NSP) for pretraining.
    """
    def __init__(self, args, vocab_size):
        super(PegasusTargetDecoderMultiInput, self).__init__()
        ...
        self.art_pointer_atten_1 = nn.Linear(args.hidden_size, args.hidden_size, bias=has_bias)
        self.art_pointer_atten_2 = nn.Linear(args.hidden_size, args.hidden_size, bias=has_bias)
    ...
    def pgn_inference(self, art_encoder_outputs, decoder_outputs, article):
        tgt_len = decoder_outputs.size(1)
        art_pointer_weights = torch.matmul(self.art_pointer_atten_1(decoder_outputs),self.art_pointer_atten_2(art_encoder_outputs).transpose(1, 2)).squeeze(2)
        art_pointer_prob = torch.softmax(art_pointer_weights, dim=2)
        art_encoder_atten_vector = torch.matmul(art_pointer_prob, art_encoder_outputs)


        alpha_prob = F.sigmoid(
            self.alpha_prob_linear(torch.cat([decoder_outputs, art_encoder_atten_vector], dim=-1)))
        vocab_prob = torch.softmax(self.mlm_linear_1(decoder_outputs), dim=-1)

        vocab_prob = alpha_prob * vocab_prob
        art_pointer_prob = (1 - alpha_prob) * art_pointer_prob
        art = article.unsqueeze(1).repeat(1, tgt_len, 1)
        vocab_prob = vocab_prob.scatter_add_(2, art, art_pointer_prob)
        return vocab_prob
    ...
    def mlm(self, art_encoder_outputs, decoder_outputs, article, tgt_mlm):
        output_mlm = self.pgn_inference(art_encoder_outputs, decoder_outputs, article)
    ...
    def forward(self, art_encoder_outputs, decoder_outputs, article, tgt_mlm):
        """
        Args:
        ...
        loss, correct, denominator = self.mlm(art_encoder_outputs, decoder_outputs, article, tgt_mlm)

        return loss, correct, denominator
    def inference_forward(self, art_encoder_outputs, decoder_outputs, article):
        output_mlm = self.pgn_inference(art_encoder_outputs, decoder_outputs, article)
