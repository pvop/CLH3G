...
from bert.transformer_addfuse_builder import build_model
...
def read_dataset(args, path):
    ...
    for i in range(len(data)):
        ...
        title = title[0:args.title_seq_length] + [PAD_ID] * (args.title_seq_length - len(title))
...
def batch_loader(args, trainset, proc_id, proc_num, test=False):
    ...
    title = torch.LongTensor([example[1] for example in procset])
    ...
    while True:
        ...
        title_batch = title[i * batch_size: (i + 1) * batch_size, :]
        tgt_batch = tgt[i * batch_size: (i + 1) * batch_size]
        unk_sets_batch = unk_sets[i * batch_size: (i + 1) * batch_size]
        i += 1
        yield art_batch, title_batch, tgt_batch, unk_sets_batch
...
def train_model(args, gpu_id, rank, loader, model, optimizer, scheduler, steps):
    ...
    while True:
        ...
        if steps == total_steps + 1:
            break
        art, title, tgt, _ = next(loader)
        if gpu_id is not None:
            art = art.cuda(gpu_id)
            title = title.cuda(gpu_id)
            tgt = tgt.cuda(gpu_id)
        loss, _, _ = model(art, title, tgt)
...
def cal_wsc_acc(args, candidate_datas):
    gold_datas = open(args.dev_path).read().split("\n")
    gold_datas = ["".join(x.lower().split()[0]) for x in gold_datas]
    labels = open(args.dev_label_path).read().split("\n")
    candidate_datas = ["".join(x.replace("[unused2]", "").replace("##", "").lower()) for x in candidate_datas]
    total = 0
    correct = 0
    for i in range(len(gold_datas)):
        if candidate_datas[i] in gold_datas[i] or gold_datas[i] in candidate_datas[i]:
            pred = "true"
        else:
            pred = "false"
        if pred==labels[i]:
...
def evaluate(args,model, gpu_id, dataset, data_path):
    ...
    t1 = time.time()
    for index, (art_batch, title_batch, tgt_batch, unk_sets) in enumerate(batch_loader(args, dataset, 0, 1, test=True)):
        if index%100==0:
        ...
        with torch.no_grad():
            pred_tokens = model.inference_forward_beam_search(art_batch, title_batch)
...
def main():
    ...
    parser.add_argument("--title_seq_length", type=int, default=256,
                        help="Sequence length.")
